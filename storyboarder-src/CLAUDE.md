...
- 마우스 커서(m)는 원시 문서좌표 x,y에 **셀/페이지 앵커 동봉**(`m.c={f,cut,block,nth,fx,fy}` 또는 `m.p={i,fx,fy}`, 송신 측 elementFromPoint) — 수신 측이 앵커를 우선 해석해 사용자별 레이아웃(열 너비·용지 방향)이 달라도 같은 내용 위에 표시(캐럿과 같은 원리, 2026-08 '타인 마우스 위치 어긋남' 수정). 앵커 해석 실패·구버전 페이로드는 x,y 폴백. SVG 화살표 꼭짓점(1.5,1)은 svg margin으로 보정.
...
- **촬영 순서 PDF**(개구리 전용, 파일 메뉴 #b-shootpdf — 뷰어도 사용 가능): Next 열(라벨 /^next$/i인 텍스트 열)의 숫자 = 같은 씬에서 다음에 찍을 컷 번호(C#). 씬 그룹(비어 있지 않은 S#가 달라지는 지점 기준)마다 그래프 순회로 촬영 순서를 계산 — 시작점은 **아무도 Next로 참조하지 않는 컷**(문서 순서 우선; 보통 1컷, 씬3처럼 선촬영이면 그 컷이 자동 뿌리), Next 파싱은 **숫자 외 모든 문자(공백·-·~·쉼표 등)를 단순 구분자**로 취급해 숫자 덩어리를 적힌 순서대로 사용("1-4 14" → 1,4,14 · "1~3" → 1,3, 범위 전개 없음; 컷 색인도 C#의 첫 숫자 덩어리 기준), 여러 번호는 그 순서대로 큐에 이어붙이고(BFS), **셀에 자기 컷 번호가 포함된 시퀀스 표기**(예: 컷 1·7·9가 모두 "1 7-9-11")면 자기 뒤 번호들만 후속으로 취급(앞 번호는 선행 컷이므로 참조로 안 셈 — 시작점 판정이 안 무너짐), 시작점이 여럿이면 문서 순서대로 하나씩 끝까지 순회, 도달 못 한 컷은 원래 순서로 뒤에. (실데이터 씬8 "1 7-9-11" → 1,7,9,11,2,3,8,12,5,6,4,10 검증됨) S# 라벨은 행 위치에 남기고 합쳐진 메모 행은 제자리 고정. 인쇄 동안만 doc.blocks를 재배열 **사본**으로 교체(shootOrderBlocks, markDirty 없음 — 발행 안 됨) 후 window.print, finally에서 복원. printSwap 플래그는 isInteracting()에 포함되어 원격 병합이 연기됨.
- **개구리 전용 스타일**(body.film-frog): Notes 셀 서체 IBM Plex Mono(한글 폴백 Pretendard). 헤더행 글자 크기 `settings.hdrFs`(pt, 3~20, 기본 5.5 — 문서 메뉴 #hdrfs-row 입력)·헤더행 높이 `settings.hdrH`(mm, 4~30, 기본 8 — 헤더 하단 .hgrip 드래그, 라이브 미리보기는 --hdr-h 변수만 갱신하고 릴리즈 시 커밋)는 문서 공유 설정으로 `--hdr-fs`/`--hdr-h` CSS 변수로 모든 페이지에 적용되고 measureBoard가 실측하므로 페이지네이션도 재계산. hdrDrag는 isInteracting()에 포함. 그립은 편집자 전용(:not(.viewer))·모바일 제외.
