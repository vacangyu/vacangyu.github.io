...
- **촬영 순서 PDF**(개구리 전용, 파일 메뉴 #b-shootpdf — 뷰어도 사용 가능): Next 열(라벨 /^next$/i인 텍스트 열)의 숫자 = 같은 씬에서 다음에 찍을 컷 번호(C#). 씬 그룹(비어 있지 않은 S#가 달라지는 지점 기준)마다 그래프 순회로 촬영 순서를 계산 — 시작점은 **아무도 Next로 참조하지 않는 컷**(문서 순서 우선; 보통 1컷, 씬3처럼 선촬영이면 그 컷이 자동 뿌리), 한 셀의 여러 번호는 그 순서대로 큐에 이어붙이고(BFS), 도달 못 한 컷은 원래 순서로 뒤에. S# 라벨은 행 위치에 남기고 합쳐진 메모 행은 제자리 고정. 인쇄 동안만 doc.blocks를 재배열 **사본**으로 교체(shootOrderBlocks, markDirty 없음 — 발행 안 됨) 후 window.print, finally에서 복원. printSwap 플래그는 isInteracting()에 포함되어 원격 병합이 연기됨.
