...
- 편집자(로그인): 변경 후 트레일링 2초·최대 6초 디바운스로 자동 발행, 성공 시 `dirty=false`(창닫기 경고 해제), "● 클라우드 저장됨" 표시.
...
- 팔로우 중에는 내 마우스(m)를 presence에 싣지 않음. 뷰포트 앵커는 기하 스캔이며 **frac은 컷 전체(슬라이스 합산) 기준** — 조각 내 frac을 수신 측이 첫 조각에 적용해 팔로워가 한 페이지 위로 튀던 버그의 원인. 페이지 하단 여백·문서 끝에서는 그 페이지 마지막 행의 끝을 앵커로(다중 페이지 블록의 block 앵커는 첫 페이지로 튐). 왕복(anchor→wantTop) 오차: 콘텐츠 위 0px.
